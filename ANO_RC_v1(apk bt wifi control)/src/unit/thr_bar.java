...
public class thr_bar extends View {
	...
	static boolean isinrect(float x1, float y1, float top, float height, float left, float width) {// x2 y2为中心点
		if (x1 > left && x1 < left + width)
			if (y1 > top && y1 < top + height) return true;
			else return false;
		else return false;
	}
